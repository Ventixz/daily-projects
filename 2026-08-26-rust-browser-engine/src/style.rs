//! Builds a "style tree": the DOM tree paired with the CSS declarations that
//! apply to each node, after matching selectors and resolving the cascade
//! by specificity (ties broken by source order, since `matching_rules`
//! walks the stylesheet in the order it was parsed).
